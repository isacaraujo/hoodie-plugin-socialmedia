...
module.exports = function (hoodie) {
  ...
  _findNotification = function (task, db, cb) {
    var _db = new ExtendedDatabaseAPI(hoodie, hoodie.database(db));
    _db.findAll('notification', function (err, notifications) {
      if (err && err.error === 'not_found') return cb(null, task);
      if (err) return cb(err);
      notifications = notifications.filter(function (item) {
        return item.from == task.socialmedia.userId && 
          item.notificationType == 'requestFriend' && 
          item.status == 'new';
      });
      task.socialmedia.notifications = notifications;
      return cb(null, task);
    });
  };

  var _getRelation = function (task, db, cb) {
    var taskb = { socialmedia: { userId: db.split('/').pop() } },
        dbb = 'user/' + task.socialmedia.userId,
        relation = [];
    async.series([
      async.apply(_findNotification, task,  db),
      async.apply(_findNotification, taskb, dbb)
    ], function (error, results) {
      if (error) return cb(error);
      results.forEach(function (item) {
        while (item.socialmedia.notifications.length) {
          var n = item.socialmedia.notifications.pop();
          relation.push(n);
        }
      });
      delete task.socialmedia.notifications;
      task.socialmedia.relation = relation;
      return cb(null, task);
    });
  };
...
//        async.apply(_verifyAttrs, task, 'userId'),
  ...
  SocialMedia.getRelation = function (db, task) {
    log('getRelation', task);

    async.series([
        async.apply(_verifyAttrs, task, 'socialmedia'),
        async.apply(_verifyAttrs, task.socialmedia, 'userId'),
        async.apply(_getRelation, task, db)
      ],
      utils.handleTask(hoodie, 'getRelation', db, task)
    );
  };
